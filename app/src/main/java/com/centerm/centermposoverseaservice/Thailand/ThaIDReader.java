...
//    private List<String> religions = Arrays.asList("ไม่นับถือศาสนา", "พุทธ", "อิสลาม", "คริสต์", "พราหมณ์-ฮินดู", "ซิกข์", "ยิว", "เชน", "โซโรอัสเตอร์", "บาไฮ", "ไม่ระบุ");
    ...
    private void sendCommandForInfo() throws RemoteException {
        ...
        String xyz = sendApdu(_cmd + _thai_id_card + "64");
        Log.e("panya", "data : " + xyz);
    ...
    private String getChipCardId() throws RemoteException {
        byte[] selectData = sendApdu2("00a40400"); //select data
        byte[] data = sendApdu2("80ca9f7f00");
        return byteArrayToHexString(data, 13, 8);
    }

    private String getLaserId() throws RemoteException {
        sendApdu2("00a4040008a000000084060002");
        byte[] data = sendApdu2("80000000FF");

        ChipCardADM chipCardADM = new ChipCardADM();

        chipCardADM.Version = String.format(Locale.US, "%d.%d", data[0], data[1]);
        chipCardADM.State = String.format(Locale.US, "%d", data[2]);
        chipCardADM.Authorize = String.format(Locale.US, "%d", data[3]);
        byte[] number = Arrays.copyOfRange(data, 7, 23);
        chipCardADM.LaserNumber = new String(number);
        been.setLaserNumber(chipCardADM.LaserNumber);

        return " version : " + chipCardADM.Version + " \n" +
                " State : " + chipCardADM.State + " \n" +
                " Authorize : " + chipCardADM.Authorize + " \n" +
                " LaserNumber : " + chipCardADM.LaserNumber;
    }

    private String getBioData() throws RemoteException {
        sendApdu2("00a4040008a000000084060000");
        byte[] data = sendApdu2("b0340076FF");

        Log.e("panya", "data : " + byteArrayToHexString(data));

        byte[] param = new byte[0x76];
        System.arraycopy(data, 0, param, 0, 0x76);

        Log.e("panya", "param : " + param + " "+byteArrayToHexString(data));
    ...
    private byte[] sendApdu2(String apdu) throws RemoteException {
        return iccard.sendAsync(HexUtil.hexStringToByte(apdu));
    }

    private String sendApdu(String apdu) throws RemoteException {
        byte[] data = iccard.sendAsync(HexUtil.hexStringToByte(apdu));
        return new String(data, _UTF8_CHARSET);
    }
    ...
    public void readIDCard(ResultCallBack callBack) throws RemoteException, DeviceBaseException {
        try {
            ...
            if (readyToRead(callBack)) {
                sendCommandForInfo();
                sendCommandForPhoto();
                String chipCardId = getChipCardId();
                been.setChipNo(chipCardId);
                String laserData = getLaserId();
                been.setDataX(been.getBp1no() + "\n" +
        ...
        switch (_index) {
            ...
            case 13: {
                if (_xx != null | _xx.length() != 0) {
                    ...
                    been.setBp1no(_xx.substring(9, _xx.indexOf("/") + 9));
                    if ("1".equals(sex)) {
                        been.setGender("M");
                    } else {
                        been.setGender("F");
